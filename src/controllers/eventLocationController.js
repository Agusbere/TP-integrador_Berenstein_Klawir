...
export const getAllEventLocationsWithoutLimit = async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
        event_locations.*,
        ...
        provinces.display_order AS province_display_order
      FROM event_locations
      INNER JOIN locations ON event_locations.id_location = locations.id
      INNER JOIN provinces ON locations.id_province = provinces.id
    `);
    ...
    console.error("Error al obtener las ubicaciones de eventos:", error);
    res.status(500).json({ message: "Error al obtener las ubicaciones de eventos" });
  }
};
...
export const updateEventLocation = async (req, res) => {
  try {
    ...
    if (location.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Ubicación no encontrada o no autorizada" });
    }
    const {
      id_location,
      name,
    ...
    if (updateFields.length > 0) {
      const updateQuery = `UPDATE event_locations SET ${updateFields.join(
        ", "
      )} WHERE id = $${paramIdx}`;
